...
//             Type::App { typ, args } => {
//                 self.get_function_args(instance(self.tctx, args.clone(),
// *typ.clone()))             }
//             Type::Alias(for_type) =>
// self.get_function_args(self.tctx.resolve_alias(for_type)),             _ =>
// panic!("Should be a function"),         }
//     }

//     fn get_struct_type(&self, name: &str) -> (DefId, Vec<(String, Type)>) {
//         fn get_struct_type_impl(tctx: &TypCtx, t: Type) -> (DefId,
// Vec<(String, Type)>) {             match t {
//                 Type::Struct { def, fields } => (def, fields.clone()),
//                 Type::Scheme { typ, .. } => get_struct_type_impl(tctx,
// *typ.clone()),                 Type::App { typ, args } => {
//                     get_struct_type_impl(tctx, instance(tctx, args.clone(),
// *typ.clone()))                 }
//                 Type::Alias(for_type) => get_struct_type_impl(tctx,
// tctx.resolve_alias(for_type)),                 _ => panic!("expected a struct
// type"),             }
//         }
...
//                 Type::Scheme { typ, .. } => implementation(tctx,
// *typ.clone()),                 Type::App { typ, args } =>
// implementation(tctx, instance(tctx, args.clone(), *typ)),
...
//         fn get_adt_type_impl(tctx: &TypCtx, t: Type) -> (DefId, Vec<(String,
// Type)>) {             match t {
//                 Type::ADT { def, constructors } => (def,
// constructors.clone()),                 Type::Scheme { typ, .. } =>
// get_adt_type_impl(tctx, *typ.clone()),                 Type::App { typ, args
// } => {                     get_adt_type_impl(tctx, instance(tctx,
// args.clone(), *typ.clone()))                 }
//                 Type::Alias(for_type) => get_adt_type_impl(tctx,
// tctx.resolve_alias(for_type)),                 _ => panic!("expected a struct
// type"),             }
//         }
//         let t = self.tctx.get_type(name).unwrap();
...
//     fn emit_expr(&mut self, ast::Expr { id, kind: expr }: &ast::Expr) ->
// Address {         match expr {
...
//                     if i == exprs.len() - 1 {
//                         let res_addr =
// self.new_temporary(self.is_ref(&e_res));
// self.current_block
...
//                 // a.b[10].c.d[20] = 1
//                 // but we would need to check it when the interpreter is
// ready                 let lval = self.emit_lval(lval);
//                 let rval = self.emit_expr(rval);
//                 match lval {
//                     Lval::Address(addr) => {
//                         self.current_block
//                             .push(Instruction::CopyAssign(addr.clone(),
// rval));                         addr
//                     }
...
// Address::ConstantString(v.clone()),             ast::ExprKind::FloatConst(v)
// => Address::ConstantFloat(*v),             ast::ExprKind::BoolConst(v) =>
// Address::ConstantBool(*v),
// ast::ExprKind::Name(ast::Name::Unresolved { path, identifier }) if
// path.is_empty() => {                 let n = identifier;
//                 if let Some(addr) = self.find_in_scopes(n) {
//                     return addr;
//                 }
//                 if let Some(pos) = self.args.iter().position(|(arg, _)| arg
// == n) {                     return Address::Parameter(pos);
//                 }
...
//                 let fields =
// self.get_struct_fields(self.env.get(&lhs_id).unwrap());                 let
// (offset, field_t) = fields                     .iter()
//                     .enumerate()
...
//             } if path.is_empty() => {
//                 // TODO:
//                 // We should believe into scope analysis but I am pretty sure
// I did not                 // do it and this might create a struct when we
// dont want to because we                 // reassigned the name of the type to
// something else                 let name = identifier.clone();
...
//                     typ: tid,
//                 });
//                 let offset_and_value = fields.iter().enumerate().map(|(i,
// (name, _))| {                     let expr =
// values.get(name).unwrap().clone();                     (i, expr)
//                 });
...
//                 let fields_expr: HashMap<_, _> =
// fields.iter().cloned().collect();
// self.current_block.push(Instruction::AllocCase {                     res:
// adt_address.clone(),                     size: fields.len(),
//                     case: cpos,
//                     typ: tid,
//                 });
...
//                     self.current_block
//                         .push(Instruction::CopyAssign(res.clone(),
// else_res));                     self.close_scope();
//                 }
//                 self.current_block.push(Instruction::Label(exit_label.0));
//                 res
//             }
...
//                     None => panic!("unexpeced type that cannot be told if its
// ref"),                     Some(true) => IsRef::Ref,
//                     Some(false) => IsRef::NoRef,
//                 };
...
//                 let Type::Array(inner_t) = self.env.get(&lhs_id).unwrap()
// else {                     panic!(
...
//                 let labels = cases.iter().map(|_|
// self.new_label()).collect::<Vec<_>>();                 let exit_label =
// self.new_label();                 for (case, label) in
// cases.iter().zip(labels.iter()) {
// self.emit_pattern(expr.clone(), &case.0, label.clone());
// let body = self.emit_expr(&case.1);                     self.current_block
...
//     fn emit_pattern(&mut self, to_match: Address, pattern: &ast::Pattern,
// next_label: Label) {         let id = pattern.id;
//         match &pattern.kind {
...
// struct_fields.iter().enumerate() {                     let field_type = if
// let Type::Alias(for_t) = field_type {
// self.tctx.resolve_alias(*for_t)                     } else {
//                         field_type.clone()
//                     };
...
//                     self.emit_pattern(val, field_pattern,
// next_label.clone());                 }
//             }
...
//                 let strct = identifier;
//                 let constructors =
// self.get_constructors(self.env.get(&id).unwrap());                 let (cpos,
// ctyp) = constructors                     .iter()
//                     .enumerate()
//                     .find(|(_, (name, _))| name == strct)
//                     .map(|(i, (_, t))| (i, t))
//                     .unwrap();
...
//                 // todo: I think this is correct as p should have a type
// assigned                 // at this point
//                 self.emit_pattern(to_match, p, next_label)
//             }
//             // TODO: I think that unit type always matches as there is only
// one possible             // value for it so there is not point in checking it
//             ast::PatternKind::Unit => {}
//             // The same for wildcard, it always matches so there is nothing
// to do for us             ast::PatternKind::Wildcard => {}
...
//             Type::Alias(for_t) =>
// self.get_constructors(&self.tctx.resolve_alias(*for_t)),
// Type::Scheme { typ, .. } => self.get_constructors(typ),             Type::App
// { typ, .. } => self.get_constructors(typ),             _ => panic!("Expected
// adt got {t:?}"),         }
//     }
...
//             Address::Name(_) => {
//                 // TODO:
//                 // Probably not every name is a ref, like, they have to be a
// global                 // name so by that we could assume they have to be
// refs but that is                 // probably a wrong assumption and the best
// way to check this                 // would be to lookup a name.
//                 // We should have a one scope structure that we can somehow
// trawerse                 // and add values to.
//                 IsRef::Ref
//             }
...
// if path.is_empty() => {                 let name = identifier;
//                 if let Some(addr) = self.find_in_scopes(name) {
//                     return Lval::Address(addr);
//                 }
//                 if let Some(pos) = self.args.iter().position(|(arg, _)| arg
// == name) {                     return Lval::Address(Address::Parameter(pos));
//                 }
...
//                 let fields =
// self.get_struct_fields(self.env.get(&lhs_id).unwrap());                 let
// offset = fields.iter().position(|(name, _)| name == field).unwrap();
